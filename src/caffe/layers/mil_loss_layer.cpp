...
void MILLossLayer<Dtype>::InferLatentLabel(
  ...
  int num = adapted_bottom_.num();
  int dim = adapted_bottom_.count() / num;
  int spatial_dim = adapted_bottom_.height() * adapted_bottom_.width();
  int channels = adapted_bottom_.channels();
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int j = 0; j < bottom[1]->height()*bottom[1]->width(); j++) {
      int k = static_cast<int>(label[i * bottom[1]->height()*bottom[1]->width() + j]);
      if (k == -1 || k == 255) {
        continue;
      }
      weak_labels[k] = 1;
...
void MILLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < num; ++i) {
    for (int j = 0; j < spatial_dim; j++) {
      const int label_value = static_cast<int>(latent_label_[i * spatial_dim + j]);
